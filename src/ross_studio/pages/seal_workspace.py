...
from ..domain import EngineeringError, SealModel
...
_DEFAULTS: dict[SealModel, dict[str, Any]] = {
    SealModel.DIRECT: {
        ...
        "kxx": [1.01e6, 1.12e6, 1.23e6],
        "kxy": [2.01e5, 2.12e5, 2.23e5],
        "kyx": [-3.01e5, -3.12e5, -3.23e5],
        "kyy": [0.0, 0.0, 0.0],
        "cxx": [1010.0, 1120.0, 1230.0],
        "cxy": [210.0, 220.0, 230.0],
        "cyx": [-310.0, -320.0, -330.0],
        "cyy": [0.0, 0.0, 0.0],
    },
    SealModel.LABYRINTH: {
        ...
        "frequency_rpm": [5000.0, 8000.0, 11000.0],
        "preswirl": 0.98,
        "gas_composition": {"Nitrogen": 0.79, "Oxygen": 0.21},
        "use_jenny_kanki": False,
    },
    SealModel.HOLE_PATTERN: {
        "shaft_diameter_mm": 196.5,
        "radial_clearance_mm": 0.18,
        "axial_length_mm": 84.9,
        "relative_roughness": 1.0e-4,
        "cell_length_mm": 2.2946,
        "cell_width_mm": 2.1,
        "cell_depth_mm": 2.8,
        "inlet_pressure_bar": 18.30,
        "outlet_pressure_bar": 8.235,
    ...
    SealModel.HYBRID: {
        ...
        "inlet_temperature_c": 26.85,
        "frequency_rpm": [2000.0, 3000.0, 5000.0],
        "gas_composition": {"Nitrogen": 0.7812, "Oxygen": 0.2096, "Argon": 0.0092},
        "hole_radial_clearance_mm": 0.30,
        "hole_axial_length_mm": 40.0,
        "hole_relative_roughness": 1e-4,
...
class SealStudioPage(QWidget):
    """Preview -> Apply workspace for native ROSS 2.3.0 seal models."""
    ...
    def __init__(self, project: ProjectModel, parent: QWidget | None = None) -> None:
        ...
        title = QLabel("Seal Studio · Native ROSS 2.3.0")
        title.setObjectName("cardHeader")
        h.addWidget(title)
        note = QLabel(
            "Direct K/C, LabyrinthSeal, HolePatternSeal and HybridSeal. Calculate is preview-only; "
            "Apply rechecks the exact input fingerprint before persisting the native-model origin. "
            "Engineering geometry is entered in mm, pressure in bar, temperature in °C and speed in rpm; "
            "the ROSS boundary converts to m, Pa, K and rad/s."
        )
        ...
        if engineering is not None:
            for index, spec in enumerate(engineering.seals):
                self.seal.addItem(
                    f"#{index + 1} {spec.name} · x={spec.position_mm:g} mm",
                    index,
                )
        ...
        self.parameters.setHorizontalHeaderLabels(["Engineering parameter / unit", "Value"])
        ...
        self.summary = QLabel(
            "Calculate a seal to expose K/C, leakage, pressure, convergence and thermodynamic-backend evidence."
        )
        self.summary.setWordWrap(True)
        summary_layout.addWidget(self.summary)
        self.kc = QTableWidget(0, 10)
        self.kc.setHorizontalHeaderLabels(
            ["rpm", "Kxx", "Kxy", "Kyx", "Kyy", "Cxx", "Cxy", "Cyx", "Cyy", "Leakage kg/s"]
        )
    ...
    def _load_existing(self) -> None:
        ...
        row = self.model.findData(
            spec.model.value if isinstance(spec.model, SealModel) else str(spec.model)
        )
        if row >= 0:
    ...
    def _load_inputs(self) -> None:
        ...
        existing = (
            spec.metadata.get("engineering_input")
            if getattr(spec, "model", None) == model
            else None
        )
        values = dict(existing) if isinstance(existing, dict) else deepcopy(_DEFAULTS[model])
        if model is SealModel.DIRECT and not isinstance(existing, dict):
            rows = len(values["frequency_rpm"])
            for key in ("kxx", "kyy", "kxy", "kyx", "cxx", "cyy", "cxy", "cyx"):
                values[key] = [float(getattr(spec, key))] * rows
    ...
    def _input_values(self) -> dict[str, Any]:
        ...
        for row in range(self.parameters.rowCount()):
            ...
            if key_item is None or value_item is None:
                continue
            result[key_item.text().strip()] = _decode(value_item.text())
        return result
    ...
    def _calculate(self) -> None:
        ...
        self.state.setText(
            f"Preview ready · {result.source_model.value} · convergence validated"
        )
        interface = (
            ""
            if result.interface_pressure_pa is None
            else f" · interface={result.interface_pressure_pa:,.3f} Pa"
        )
        iterations = (
            "" if result.iterations is None else f" · iterations={result.iterations}"
        )
        residual = (
            "" if result.residual is None else f" · residual={result.residual:.3e}"
        )
        backend_text = ""
        if result.fluid_backend:
            backend = result.fluid_backend
            backend_text = (
                f" · EOS requested={backend.get('requested_eos')}"
                f" · configured={backend.get('configured_eos')}"
                f" · THERMODYNAMIC BACKEND={backend.get('actual_backend')}"
                f" · backend-status={backend.get('status')}"
            )
            if backend.get("warning"):
                backend_text += f" · WARNING: {backend['warning']}"
        native_warning_text = ""
        if result.native_warnings:
            native_warning_text = " · Native warning(s): " + " | ".join(
                result.native_warnings
            )
        ...
        for row, point in enumerate(result.coefficients):
            leakage = (
                result.leakage_kg_s[min(row, len(result.leakage_kg_s) - 1)]
                if result.leakage_kg_s
                else float("nan")
            )
            values = (
                point.rpm,
    ...
    def _collect_figures(self, native: Any) -> None:
        ...
        for label, coefficients in (
            ("Stiffness K vs frequency", ["kxx", "kyy", "kxy", "kyx"]),
            ("Damping C vs frequency", ["cxx", "cyy", "cxy", "cyx"]),
        ):
            try:
                self.figures[label] = native.plot(
        ...
        if hasattr(native, "plot_pressure_distribution"):
            try:
                self.figures["Pressure distribution"] = native.plot_pressure_distribution(
                    pressure_units="MPa",
                    length_units="mm",
                )
            except Exception as exc:
        ...
        if self.figures:
            ...
            self.plot.set_unavailable(
                "This native ROSS seal exposes no compatible plot for the solved preview."
            )
        if errors:
            self.state.setText(
                "Preview calculated; native plot failures: " + "; ".join(errors)
            )
    ...
    def _apply(self) -> None:
        ...
        try:
            engineering = self._engineering()
            index = int(self.seal.currentData())
            live_fingerprint = self.service.input_fingerprint(
                engineering,
                index,
                self._current_model(),
                self._input_values(),
            )
            if live_fingerprint != self.preview.input_fingerprint:
                self._invalidate_preview()
                raise EngineeringError(
                    "Seal Apply rejected: inputs changed after Calculate. "
                    "Expected the exact preview fingerprint; recalculate before Apply."
                )
        ...
        self.apply.setEnabled(False)
        self.state.setText(
            "Applied · native seal reconstruction passed strict ROSS build"
        )
